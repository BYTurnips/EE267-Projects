...
var StereoUnwarpRenderer = function ( webglRenderer, dispParams ) {
    ...
    this.renderTargetL.depthTexture = new THREE.DepthTexture(); // Added
	...
	this.renderTargetR = new THREE.WebGLRenderTarget(
		dispParams.canvasWidth, dispParams.canvasHeight );
    this.renderTargetR.depthTexture = new THREE.DepthTexture(); // Added
	...
	function setUnwarpMaterial( renderTarget, centerCoord ) {
		...
		var material = new THREE.RawShaderMaterial( {
			uniforms: {
				textureMap: { value: renderTarget.texture },
                depthMap: { value: renderTarget.depthTexture },
	...
	function render( state ) {
		...
		webglRenderer.setScissor(
			0, 
            0, 
            dispParams.canvasWidth / 2, 
            dispParams.canvasHeight );
